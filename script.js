    ...
    if (!name) { alert("Proszę podać nazwę własnego składnika."); customModalIngredientNameInput.focus(); return; }
    if (isNaN(calories) || calories < 0) { alert("Proszę podać poprawną liczbę kalorii."); customModalIngredientCaloriesInput.focus(); return; }
    ...
    alert(`Składnik "${name}" (${calories} kcal/100g) został ${isNew ? 'dodany jako własny' : 'zaktualizowany'}.`);
    ...
    if (!wybranaData) { alert("Proszę wybrać datę."); return; }
    ...
    alert("Błąd wczytywania pliku."); console.error(err);
