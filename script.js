...
// --- 2. BAZA SKŁADNIKÓW i LOCALSTORAGE ---
const poczatkowaBazaSkladnikow = {
    "platki owsiane":{kalorieNa100g:350},"mleko 2%":{kalorieNa100g:50},"napoj migdalowy":{kalorieNa100g:24},"banan":{kalorieNa100g:89},
    "jablko":{kalorieNa100g:52},"gruszka":{kalorieNa100g:57},"mandarynka":{kalorieNa100g:53},"orzechy wloskie":{kalorieNa100g:654},
    "migdaly":{kalorieNa100g:579},"rodzynki":{kalorieNa100g:299},"kakao (proszek)":{kalorieNa100g:228},"nasiona chia":{kalorieNa100g:486},
    "miod":{kalorieNa100g:304},"jogurt naturalny":{kalorieNa100g:61},"chleb pelnoziarnisty":{kalorieNa100g:250},"awokado":{kalorieNa100g:160},
    "jajko":{kalorieNa100g:143},"kasza peczak (sucha)":{kalorieNa100g:337},"kasza jeczmienna (sucha)":{kalorieNa100g:354},"kasza bulgur (sucha)":{kalorieNa100g:342},
    "kasza gryczana (sucha)":{kalorieNa100g:343},"kasza jaglana (sucha)":{kalorieNa100g:378},"ryz brazowy (suchy)":{kalorieNa100g:360},
    "makaron pelnoziarnisty (suchy)":{kalorieNa100g:350},"maka pszenna":{kalorieNa100g:364},"ziemniaki":{kalorieNa100g:77},"losos (surowy)":{kalorieNa100g:208},
    "dorsz (surowy)":{kalorieNa100g:82},"krewetki (gotowane)":{kalorieNa100g:99},"tofu naturalne":{kalorieNa100g:76},
    "soczewica czerwona (sucha)":{kalorieNa100g:353},"szpinak":{kalorieNa100g:23},"brokul":{kalorieNa100g:34},"papryka czerwona":{kalorieNa100g:31},
    "marchew":{kalorieNa100g:41},"cukinia":{kalorieNa100g:17},"cebula":{kalorieNa100g:40},"pomidor":{kalorieNa100g:18},
    "oliwa z oliwek":{kalorieNa100g:884},"ser feta":{kalorieNa100g:264},"czosnek":{kalorieNa100g:149},"kefir naturalny":{kalorieNa100g:51},
    "maslanka naturalna":{kalorieNa100g:40},"truskawki":{kalorieNa100g:32},"pomarancza":{kalorieNa100g:47},"maslo orzechowe":{kalorieNa100g:588},
    "tunczyk w sosie wlasnym (odsaczony)":{kalorieNa100g:100},"salata lodowa":{kalorieNa100g:14},"ogorek zielony":{kalorieNa100g:15},
    "kukurydza konserwowa":{kalorieNa100g:86},"rzodkiewka":{kalorieNa100g:16},"oliwki zielone":{kalorieNa100g:145},"pieczarki":{kalorieNa100g:22}
};
let bazaSkladnikow = JSON.parse(JSON.stringify(poczatkowaBazaSkladnikow));
let listaSkladnikow = Object.keys(bazaSkladnikow).sort();

function wczytajCustomIngredientsLocal() {
    let saved = JSON.parse(localStorage.getItem("customIngredients") || "{}");
    for (const [name, kcal] of Object.entries(saved)) {
        bazaSkladnikow[name] = { kalorieNa100g: kcal, wlasny: true };
        if (!listaSkladnikow.includes(name)) listaSkladnikow.push(name);
    }
    listaSkladnikow.sort();
}

function zapiszCustomIngredientDoLocalStorage(name, kcal) {
    let saved = JSON.parse(localStorage.getItem("customIngredients") || "{}");
    saved[name] = kcal;
    localStorage.setItem("customIngredients", JSON.stringify(saved));
}
...
// --- 4. PUSTE STANY ---
function pokazPustyStan(typPosilku) {
    ...
    if (container && emptyDiv) {
        if (container.children.length === 0) {
            emptyDiv.textContent = "Kliknij 'Dodaj Składnik', aby rozpocząć.";
        } else {
            emptyDiv.textContent = "";
        }
    }
}
function pokazPustyStanZakupow() {
    const ul = document.getElementById('shopping-list-display');
    const emptyDiv = document.getElementById('empty-shopping-list');
    if (ul && emptyDiv) {
        if (ul.children.length === 0) {
            emptyDiv.textContent = "Twoja lista zakupów jest pusta.";
        } else {
            emptyDiv.textContent = "";
        }
    }
...
// --- 6. MODALE, BUTTONY, OBSŁUGA WŁASNEGO SKŁADNIKA ---
...
addCustomIngredientConfirmButton.addEventListener('click', () => {
    ...
    // LocalStorage: zapis
    zapiszCustomIngredientDoLocalStorage(name, calories);
...
// --- 7. DODAWANIE, USUWANIE, PRZELICZANIE SKŁADNIKÓW i KALORII ---
function dodajSkladnik(typPosilku, skladnikData = null, kalkulujOdRazu = true) {
    ...
    pokazPustyStan(typPosilku);
}

function usunSkladnik(rowElement, typPosilku) {
    if (rowElement) { rowElement.remove(); przeliczKalorie(typPosilku); pokazPustyStan(typPosilku); }
}
...
function aktualizujSumeDniaCala() {
    ...
    if (progressBar) {
        let percent = Math.min((sumaCalkowita / dziennyLimitKalorii) * 100, 120);
        progressBar.style.width = percent + "%";
        if (sumaCalkowita > dziennyLimitKalorii) {
...
confirmSaveDateButton.addEventListener('click', () => {
    ...
    showToast("Jadłospis zapisany!", "success");
});
    ...
    if (file) {
        ...
        reader.onload = (e) => {
            try {
                ...
                showToast("Jadłospis wczytany!", "success");
            } catch (err) {
                showToast("Błąd wczytywania pliku!", "error"); console.error(err);
...
window.onload = function() {
    ...
    // Ustaw puste stany
    typyPosilkow.forEach(typ => pokazPustyStan(typ));
    pokazPustyStanZakupow();
};